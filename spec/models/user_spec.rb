...
describe User do
  before(:each) do
    @attr = { :name => "Example User", :email => "user@example.com",
    				:loginId => "THE_MAN", :password => "foobar",
  ...
  it "should require a login id" do
    no_email_user = User.new(@attr.merge(:loginId => ""))
    no_email_user.should_not be_valid
  end
  
  it "should reject login ids that are too long" do
    long_id = "a" * 33
    long_id_user = User.new(@attr.merge(:loginId => long_id))
    long_id_user.should_not be_valid
  end
  ...
  describe "broment associations" do
    ...
    describe "status feed" do
      ...
      it "should include the user's broments" do
        @user.feed.should include(@mp1)
        @user.feed.should include(@mp2)
      end

      it "should not include a different user's broments" do
        mp3 = Factory(:broment,
                      :user => Factory(:user, :email => Factory.next(:email), :loginId => Factory.next(:loginId)))
        @user.feed.should_not include(mp3)
      end

      it "should include the broments of followed users" do
        followed = Factory(:user, :email => Factory.next(:email),
        					 :loginId => Factory.next(:loginId))
        mp3 = Factory(:broment, :user => followed)
